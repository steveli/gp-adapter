...
class GPNet(object):
    ...
    def extend_network(self, mu, draw_sample):
        if not draw_sample:
            ...
            cov_zs = self.post_gp.cov_rand_proj(
                    n_sample=batch_size, n_lanczos_basis=self.n_lanczos_basis)
    ...
    def train(self, x, y, label):
        ...
        for epoch in xrange(self.n_epochs):
            for i, ((idx_train, w_train), each_y, each_label) in enumerate(
                    izip(idx_w, y, label)):
                history[i].append(self.train_fn(idx_train, w_train,
                                                self.idx_test, self.w_test,
                                                each_y, [each_label]))
...
def main():
    ...
    parser.add_argument('-b', dest='lanczos_basis', type=int, default=5,
